...
from ripple import core
from ripple.constants import LOCK_FILENAME


class QuietTestCase(unittest.TestCase):
    """Silence the reporting helpers so test output stays readable."""

    extra_quiet: tuple = ()

    def setUp(self):
        names = ("step", "info", "ok", "warn") + self.extra_quiet
        self._patches = [patch.object(core, n) for n in names]
...
class ConfigTests(unittest.TestCase):
    def test_round_trip(self):
        cfg = core.Config(central_base=Path("/store"), enabled_sources=["ge-proton"], manage_umu=True)
        self.assertEqual(core.Config.from_dict(cfg.to_dict()), cfg)
    ...
    def test_config_without_manage_umu_is_still_usable(self):
        ...
        cfg = core.Config.from_dict({"central_base": "/store", "enabled_sources": ["ge-proton"]})
    ...
    def test_corrupt_config_warns_instead_of_failing_silently(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.object(core, "CONFIG_PATH", path), patch.object(core, "warn") as warn:
                self.assertIsNone(core.load_config())
            self.assertTrue(warn.called)

    def test_missing_config_is_not_an_error(self):
        with tempfile.TemporaryDirectory() as tmp:
            with patch.object(core, "CONFIG_PATH", Path(tmp) / "absent.json"), patch.object(core, "warn") as warn:
                self.assertIsNone(core.load_config())
...
class SpecParsingTests(unittest.TestCase):
    def test_valid_spec(self):
        self.assertEqual(core.parse_spec("ge-proton:GE-Proton10-20"), ("ge-proton", "GE-Proton10-20"))

    def test_tag_may_contain_colons(self):
        self.assertEqual(core.parse_spec("umu:1:2"), ("umu", "1:2"))
...
class CachyosAssetTests(unittest.TestCase):
    def test_suffixes_are_ordered_best_first(self):
        with patch.object(core, "detect_cpu_level", return_value=3):
            self.assertEqual(core._cachyos_suffixes(), ["x86_64_v3", "x86_64_v2", "x86_64"])
        with patch.object(core, "detect_cpu_level", return_value=1):
            self.assertEqual(core._cachyos_suffixes(), ["x86_64"])
    ...
    def test_best_runnable_asset_is_chosen(self):
        ...
        with patch.object(core, "detect_cpu_level", return_value=3):
            self.assertEqual(core._cachyos_choose(matches), "https://example.invalid/proton-x86_64_v3.tar.xz")
        with patch.object(core, "detect_cpu_level", return_value=2):
            self.assertEqual(core._cachyos_choose(matches), "https://example.invalid/proton-x86_64_v2.tar.xz")

    def test_asset_built_for_a_newer_cpu_is_not_offered(self):
        with patch.object(core, "detect_cpu_level", return_value=2):
            self.assertFalse(core._cachyos_asset_filter("https://example.invalid/proton-x86_64_v4.tar.xz"))
            self.assertTrue(core._cachyos_asset_filter("https://example.invalid/proton-x86_64_v2.tar.xz"))
...
class CleanupTests(QuietTestCase):
    ...
    def test_latest_and_locked_survive_cleanup(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            cfg = core.Config(central_base=Path(tmp), enabled_sources=["ge-proton"])

            core.remove_old_versions(cfg, [])
    ...
    def test_only_broken_links_into_our_store_are_removed(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            store = root / "store"
            (store / "crate").mkdir(parents=True)
            targets = root / "compatibilitytools.d"
            targets.mkdir()

            ours = targets / "ge-proton-latest"
            ours.symlink_to(store / "crate" / "ge-proton" / "gone")
            theirs = targets / "someone-elses-proton"
            theirs.symlink_to(root / "elsewhere" / "gone")

            cfg = core.Config(central_base=store, enabled_sources=[])
            core.remove_old_versions(cfg, [targets])
...
class PinnedInstallTests(QuietTestCase):
    def test_pinned_install_is_locked_so_cleanup_keeps_it(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            store.mkdir()
            release = core.ReleaseInfo("GE Proton", "ge-proton", "v9", "https://example.invalid/p-x86_64.tar.gz")
            ...
            with (
                patch.object(core, "download_file", lambda url, dest, label: Path(dest).write_bytes(b"")),
                patch.object(core, "extract_archive", fake_extract),
            ):
                core.install_release(release, store, [], update_latest=False)

            version_dir = store / "crate" / "ge-proton" / "v9"
            self.assertTrue(version_dir.is_dir())
            self.assertTrue((version_dir / LOCK_FILENAME).exists())
            self.assertFalse((store / "crate" / "ge-proton" / ".latest-version").exists())

            cfg = core.Config(central_base=store, enabled_sources=["ge-proton"])
            core.remove_old_versions(cfg, [])
            self.assertTrue(version_dir.is_dir())
...
class DownloadTests(QuietTestCase):
    ...
    def test_truncated_download_is_rejected_and_cleaned_up(self):
        with tempfile.TemporaryDirectory() as tmp:
            dest = Path(tmp) / "core.tar.gz"
            with patch.object(core, "_open_url", return_value=self._Response(b"short", 100)):
                with self.assertRaises(RuntimeError):
                    core.download_file("https://example.invalid/a.tar.gz", dest, "test")
            self.assertFalse(dest.exists())

    def test_complete_download_is_kept(self):
        with tempfile.TemporaryDirectory() as tmp:
            dest = Path(tmp) / "core.tar.gz"
            with patch.object(core, "_open_url", return_value=self._Response(b"complete", 8)):
                core.download_file("https://example.invalid/a.tar.gz", dest, "test")
...
class TransportTests(unittest.TestCase):
    def test_plain_http_is_refused(self):
        with self.assertRaises(RuntimeError):
            core._require_https_url("http://example.invalid/a.tar.gz")

    def test_redirect_to_plain_http_is_refused(self):
        handler = core._HTTPSOnlyRedirectHandler()
        with self.assertRaises(RuntimeError):
